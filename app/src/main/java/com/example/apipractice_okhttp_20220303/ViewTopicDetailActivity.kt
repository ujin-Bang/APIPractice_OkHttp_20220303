...
import com.example.apipractice_okhttp_20220303.adapters.ReplyAdapter
...
//            투표를 하지 않은 상태라면, 댓글 작성도 불가하도록 하자.
            if(mTopicData.mySelectedSide == null) {
                Toast.makeText(mContext, "의견을 개진할 진영르 선택하셔야 합니다", Toast.LENGTH_SHORT).show()
...
//        btnVote01클릭 => 첫 진영의 id값을 찾아서 거기에 투표
//        서버에 전달 =>API 활용
        binding.btnVoteCount1.setOnClickListener {
...
//           투표 현황 새로고침(응답)
        ...
        binding.btnVoteCount2.setOnClickListener {
...
//        화면 초기 진입 실행 + 서버에서 다시 받아왔을 때도 실행
...
//                첫 진영에 투표한 경우
                binding.btnVoteCount1.text = "투표 취소"
                binding.btnVoteCount2.text = "다시 투표"
            }
            else {
//                두번째 진영에 투표한 경우
                binding.btnVoteCount1.text = "다시 투표"
                binding.btnVoteCount2.text = "투표 취소"
            }
        }
        else {
//            아무곳에도 투표하지 않은 경우
            binding.btnVoteCount1.text = "투표 하기"
            binding.btnVoteCount2.text = "투표 하기"
        }
...
//                토론 정보JSOBObject(topicObj) => TopicData()형태로 변환(여러 화면에서 진행. 함수로 만들어두자)
...
//                mReplyList에 넣어주자
                ...
                for (i in 0 until repliesArr.length()){
                    ...
                    mReplyList.add( ReplyData.getReplyDataFromJon(replyObj))
                }
